...
INTEGER Rcond, nchar, nv, k, nvalue
...
OPEN(unit=10,file='smooth_ssh.in')
READ(10,'(a)') pathin
READ(10,'(a)') pathout
READ(10,*) istart,istop
CLOSE(10)

OPEN(unit=11,file='ssh_files_corrupted.out')
...
filein=TRIM(pathin)//'ssh_opendap_'//num_record//'.nc'
fileout=TRIM(pathout)//'ssh_opendap_'//num_record//'.nc'
...
! Get variable
   ...
   if(status /= nf90_NoErr) call handle_err(status)
   status = nf90_get_var(nc,VarId,scr2d)
   if(status /= nf90_NoErr) call handle_err(status)

! If the file contains only zero's, move the file back into the input folder and end the program
   if ((MAXVAL(scr2d).eq. 0.).and.(MINVAL(scr2d).eq.0.))then
   write(11,*)'File : ssh_opendap_'//num_record//'.nc does not contain any data '
   CALL system('rm -f '// TRIM(fileout))
   write(*,*) 'Must re download file: ssh_opendap_'//num_record//'.nc'
   GO TO 100
   endif
...
! call smooth routine
 ...
 !
! Write array
    status = nf90_put_var(nc,VarId,scr2d,(/1,1/))
    if(status /= nf90_NoErr) call handle_err(status)        

! Close netcdf file
100 status = nf90_close(nc)
    if(status /= nf90_NoErr) call handle_err(status)
    write(*,*) 'Laplacian interpolation done for file: ssh_opendap_'//num_record//'.nc'


ENDDO
! ______________________________________________
END PROGRAM smooth_ssh

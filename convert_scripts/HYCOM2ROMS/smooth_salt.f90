...
OPEN(unit=10,file='smooth_salt.in')
READ(10,'(a)') pathin
READ(10,'(a)') pathout
READ(10,*) istart,istop
CLOSE(10)

OPEN(unit=11,file='salt_files_corrupted.out')
...
filein=TRIM(pathin)//'salt_opendap_'//num_record//'.nc'
fileout=TRIM(pathout)//'salt_opendap_'//num_record//'.nc'
...
! Get dimensions
   ...
   IF(I.eq.istart) THEN
      write(*,*) 'smooth_salt: Lp, Mp, N = ',Lp,Mp,N
...
! If the file contains only zero's, move the file back into the input folder and end the program
   if ((MAXVAL(scr3d).eq. 0.).and.(MINVAL(scr3d).eq.0.))then
      write(11,*)'File : salt_opendap_'//num_record//'.nc does not contain any data '
      CALL system('rm -f ' // TRIM(fileout))
      write(*,*) 'Must re download file: salt_opendap_'//num_record//'.nc'
      GO TO 100
   endif
...
! Write array
    ...
    write(*,*) 'Laplacian interpolation done for file: salt_opendap_'//num_record//'.nc'

...
OPEN(unit=10,file='smooth_temp.in')
READ(10,'(a)') pathin
READ(10,'(a)') pathout
READ(10,*) istart,istop
CLOSE(10)

OPEN(unit=11,file='temp_files_corrupted.out')
...
filein=TRIM(pathin)//'temp_opendap_'//num_record//'.nc'
fileout=TRIM(pathout)//'temp_opendap_'//num_record//'.nc'
...
! Get dimensions
   ...
   IF(I.eq.istart) THEN
      write(*,*) 'smooth_temp: Lp, Mp, N = ',Lp,Mp,N
...
! If the file contains only zero's, move the file back into the input folder and end the program
   ...
   write(11,*)'File : temp_opendap_'//num_record//'.nc does not contain any data '
   CALL system('rm -f '//TRIM(fileout))
   write(*,*) 'Must re-download temp_opendap_'//num_record//'.nc'
   GO TO 100
   endif
...
! Write array
    ...
    write(*,*) 'Laplacian interpolation done for file: temp_opendap_'//num_record//'.nc'

...
OPEN(unit=10,file='smooth_u.in')
READ(10,'(a)') pathin
READ(10,'(a)') pathout
READ(10,*) istart,istop
CLOSE(10)

OPEN(unit=11,file='u_files_corrupted.out')
...
filein=TRIM(pathin)//'uvel_opendap_'//num_record//'.nc'
fileout=TRIM(pathout)//'uvel_opendap_'//num_record//'.nc'
...
! Get dimensions
   ...
   IF(I.eq.istart) THEN
      write(*,*) 'smooth_u: Lp, Mp, N = ',Lp,Mp,N
...
! If the file contains only zero's, move the file back into the input folder and end the program
   ...
   write(11,*) 'File : uvel_opendap_'//num_record//'.nc does not contain any data '
   write(*,*) 'Must re-download the file: uvel_opendap_'//num_record//'.nc'
...
! Write array
    ...
    write(*,*) 'Laplacian interpolation done for file: uvel_opendap_'//num_record//'.nc'

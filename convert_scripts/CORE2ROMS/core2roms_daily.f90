...
OPEN(unit=10,file='core2roms_daily.in')
READ(10,*)
READ(10,'(a)') path_core
READ(10,*)
READ(10,'(a)') gridfile
...
nb_records = (jd(year_end,month_end,day_end)-jd(year_st,month_st,day_st)+1)

DO irec=1,nb_records

date= REAL(jd(year_st,month_st,day_st))+REAL(irec-1)*24./24.
greg=0
CALL GR(greg,date)
iyear= greg(1)
imonth= greg(2)
...
if(status_1 /= nf90_NoErr) call handle_err(status_1)
! Get SSRD var id
status_1 = nf90_inq_varid(ncid_6, "SWDN_MOD",SSRDVarId)
if(status_1 /= nf90_NoErr) call handle_err(status_1)
! Get STRD var id
status_1 = nf90_inq_varid(ncid_6, "LWDN_MOD",STRDVarId)
if(status_1 /= nf90_NoErr) call handle_err(status_1)
! Read in time value
...
! Read in SSRD values
status_CORE = nf90_get_var(ncid_6,SSRDVarId,work,start=(/1, 1,itime /))
if(status_CORE /= nf90_NoErr) call handle_err(status_CORE)
!SSRD = work*SSRD_scale_factor + SSRD_add_offset
SSRD = work
! Read in STRD values
status_CORE = nf90_get_var(ncid_6,STRDVarId,work,start=(/1, 1,itime /))
if(status_CORE /= nf90_NoErr) call handle_err(status_CORE)
...
! For CORE, no need to change units
   swrad = SSRD(i1:i2,j1:j2)
   lwrad = STRD(i1:i2,j1:j2)

end if

! Write out variables in netCDF file
if(iyear.eq.year2 .AND. imonth .eq. month2 .AND. iday .eq. day2) then

...
statuso = nf90_def_var(ncido,"cloud",nf90_float, &
                 (/ XiRhoDimID, EtaRhoDimID, TimeOutDimID /), cloudairVarId)
if(statuso /= nf90_NoErr) call handle_err(statuso)
statuso = nf90_put_att(ncido, cloudairVarId,"long_Name", &
                 "Fraction cloud cover")
if(statuso /= nf90_NoErr) call handle_err(statuso)
statuso = nf90_put_att(ncido, cloudairVarId,"units", &
                 " ")
if(statuso /= nf90_NoErr) call handle_err(statuso)
...
if(statuso /= nf90_NoErr) call handle_err(statuso)
if(yes_no.eq.0)then
statuso = nf90_put_att(ncido, cloudairVarId,"coordinates", &
                 "longitude latitude")
if(statuso /= nf90_NoErr) call handle_err(statuso)
end if
! RAIN variable
statuso = nf90_def_var(ncido,"rain",nf90_float, &
                 (/ XiRhoDimID, EtaRhoDimID, TimeOutDimID /), rainairVarId)
if(statuso /= nf90_NoErr) call handle_err(statuso)
statuso = nf90_put_att(ncido, rainairVarId,"long_Name", &
                 "Total precipiation")
if(statuso /= nf90_NoErr) call handle_err(statuso)
statuso = nf90_put_att(ncido, rainairVarId,"units", &
                 "kg meter-2 second-1")
if(statuso /= nf90_NoErr) call handle_err(statuso)
statuso = nf90_put_att(ncido, rainairVarId,"time", &
                 "time")
if(statuso /= nf90_NoErr) call handle_err(statuso)
if(yes_no.eq.0)then
statuso = nf90_put_att(ncido, rainairVarId,"coordinates", &
                 "longitude latitude")
if(statuso /= nf90_NoErr) call handle_err(statuso)
end if
! SWRAD variable
...
statuso = nf90_put_var(ncido,cloudairVarId,cloudair,start=(/1, 1, irec /))
if(statuso /= nf90_NoErr) call handle_err(statuso)

status_roms = nf90_put_var(ncido,rainairVarId,rainair,start=(/1, 1, irec/))
if(status_roms /= nf90_NoErr) call handle_err(status_roms)

...
INTEGER LonDimID, LatDimID, TimeDimId,TimeVarId, U10VarId, V10VarId
...
OPEN(unit=10,file='smooth_wind.in')
READ(10,'(a)') pathin
READ(10,'(a)') pathout
...
DO imonth= month_st,12
   ...
   DO itime= 1,ntime
      ...
      ! Read in U10 value
      ! Get U10 var id
      status_cfsr = nf90_inq_varid(ncid_cfsr, "U_GRD_L103",U10VarId)
      if(status_cfsr /= nf90_NoErr) call handle_err(status_cfsr)
      status_cfsr = nf90_get_var(ncid_cfsr,U10VarId,scr,start=(/1, 1,itime /))
      if(status_cfsr /= nf90_NoErr) call handle_err(status_cfsr)
      ...
      status_cfsr = nf90_put_var(ncid_cfsr,U10VarId,scr,(/1,1,itime/))
      if(status_cfsr /= nf90_NoErr) call handle_err(status_cfsr)   
      ! _________________________________________________________________
      ! Read in V10 value
      ! Get V10 var id
      status_cfsr = nf90_inq_varid(ncid_cfsr, "V_GRD_L103",V10VarId)
      if(status_cfsr /= nf90_NoErr) call handle_err(status_cfsr)
      status_cfsr = nf90_get_var(ncid_cfsr,V10VarId,scr,start=(/1, 1,itime /))
      if(status_cfsr /= nf90_NoErr) call handle_err(status_cfsr)
      ! Fill in masked-out values 
      scrp = 0.
      scrp = scr
      ...
      CALL smooth_polar(scrp, scr, nlon, nlat)
      ! Write out in netCDF file
      status_cfsr = nf90_put_var(ncid_cfsr,V10VarId,scr,(/1,1,itime/))
      if(status_cfsr /= nf90_NoErr) call handle_err(status_cfsr)   
   ENDDO
   ...
   if(status_cfsr /= nf90_NoErr) call handle_err(status_cfsr)
   write(*,*) 'Laplacian interpolation and Polar smoothing done for file: ','wind10m_'//TRIM(ryear)//TRIM(rmonth)//'.nc'
ENDDO

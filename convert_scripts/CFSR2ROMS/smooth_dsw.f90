...
INTEGER LonDimID, LatDimID, TimeDimId,TimeVarId, DSWVarId
...
OPEN(unit=10,file='smooth_dsw.in')
READ(10,'(a)') pathin
READ(10,'(a)') pathout
...
DO imonth= month_st,12
   ...
   filein=TRIM(pathin)//'dswsfc.gdas.'//TRIM(ryear)//TRIM(rmonth)//'.grb2.nc'
   fileout=TRIM(pathout)//'dsw_'//TRIM(ryear)//TRIM(rmonth)//'.nc'
   ...
   DO itime= 1,ntime
      ...
      ! Read in DSW value
      ! Get DSW var id
      status_cfsr = nf90_inq_varid(ncid_cfsr, "DSWRF_L1",DSWVarId)
      if(status_cfsr /= nf90_NoErr) call handle_err(status_cfsr)
      status_cfsr = nf90_get_var(ncid_cfsr,DSWVarId,scr,start=(/1, 1,itime /))
      if(status_cfsr /= nf90_NoErr) call handle_err(status_cfsr)
      ...
      status_cfsr = nf90_put_var(ncid_cfsr,DSWVarId,scr,(/1,1,itime/))
      if(status_cfsr /= nf90_NoErr) call handle_err(status_cfsr)   
   ENDDO
   ...
   if(status_cfsr /= nf90_NoErr) call handle_err(status_cfsr)
   write(*,*) 'Laplacian interpolation and Polar smoothing done for file: ','dsw_'//TRIM(ryear)//TRIM(rmonth)//'.nc'
ENDDO

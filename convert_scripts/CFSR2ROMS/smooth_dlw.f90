...
INTEGER LonDimID, LatDimID, TimeDimId,TimeVarId, DLWVarId
...
OPEN(unit=10,file='smooth_dlw.in')
READ(10,'(a)') pathin
READ(10,'(a)') pathout
...
DO imonth= month_st,12
   ...
   filein=TRIM(pathin)//'dlwsfc.gdas.'//TRIM(ryear)//TRIM(rmonth)//'.grb2.nc'
   fileout=TRIM(pathout)//'dlw_'//TRIM(ryear)//TRIM(rmonth)//'.nc'
   ...
   DO itime= 1,ntime
      ...
      ! Read in DLW value
      ! Get DLW var id
      status_cfsr = nf90_inq_varid(ncid_cfsr, "DLWRF_L1",DLWVarId)
      if(status_cfsr /= nf90_NoErr) call handle_err(status_cfsr)
      status_cfsr = nf90_get_var(ncid_cfsr,DLWVarId,scr,start=(/1, 1,itime /))
      if(status_cfsr /= nf90_NoErr) call handle_err(status_cfsr)
      ...
      status_cfsr = nf90_put_var(ncid_cfsr,DLWVarId,scr,(/1,1,itime/))
      if(status_cfsr /= nf90_NoErr) call handle_err(status_cfsr)   
   ENDDO
   ...
   if(status_cfsr /= nf90_NoErr) call handle_err(status_cfsr)
   write(*,*) 'Laplacian interpolation and Polar smoothing done for file: ','dlw_'//TRIM(ryear)//TRIM(rmonth)//'.nc'
ENDDO

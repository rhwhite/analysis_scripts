...
DO imonth= month_st,12
   ...
   DO itime= 1,ntime
      ...
      ! Read in value
      ! Get var id
      status_cfsr = nf90_inq_varid(ncid_cfsr, "SPF_H_L103",VarId)
      if(status_cfsr /= nf90_NoErr) call handle_err(status_cfsr)
   ...
   if(status_cfsr /= nf90_NoErr) call handle_err(status_cfsr)
   write(*,*) 'Check for missing values and time in hours added for: ','q2m_'//TRIM(ryear)//TRIM(rmonth)//'.nc'
ENDDO

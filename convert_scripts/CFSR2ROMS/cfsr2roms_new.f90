...
! Designed for use with 6hr forecasts for wind10, t2m, q2m; 6hr averages
! for rain, cloud, dlw and dls, and 00 analysis for msl.
! No interpolation onto ROMS grid (this means msl is written out on a 
! different grid currently. The code for this has been left in, but will need
! to be checked before use.
! Uses data run through the 'process' scripts - which create a Julian time and
! change the name of the file!
! Time stamp is altered for the averages, to give a time stamp half-way through
! the 6-hourly average time. This is hard-coded to 6hours.
!
PROGRAM cfsr2roms
use netcdf
implicit none
...
INTEGER XiDimID, EtaDimID, Lp, Mp, Lp_dim, Mp_dim, Lp_dim_msl, Mp_dim_msl
...
INTEGER TimeOutVarIdwind, FCVarID_wind
INTEGER TimeOutVarIdmsl, FCVarID_msl
INTEGER TimeOutVarIdq2m, FCVarID_q2m
INTEGER TimeOutVarIdt2m, FCVarID_t2m
INTEGER TimeOutVarIdcloud, FCVarID_cloud
INTEGER TimeOutVarIdrain, FCVarID_rain
INTEGER TimeOutVarIddsw, FCVarID_dsw
INTEGER TimeOutVarIddlw, FCVarID_dlw
...
INTEGER FC_wind, FC_msl, FC_t2m, FC_q2m, FC_cloud, FC_rain, FC_dsw, FC_dlw
...
OPEN(unit=10,file='cfsr2roms_new.in')
READ(10,*)
READ(10,'(a)') path_cfsr
READ(10,*)
READ(10,'(a)') gridfile
READ(10,*)
READ(10,'(a)') fileout
yes_no = 0 ! No interpolation onto ROMS grid. This can be changed, but the
! code will need to be checked thoroughly. There is little point in doing this!
...
date= REAL(jd(year_st,month_st,day_st))+REAL(irec-1)*(6.d0/24.d0) + 0.00001
greg=0
CALL GR(greg,date)
iyear= greg(1)
imonth= greg(2)
...
call write_err("unexpected ihour")
end if
...
!Wind file
statuso = nf90_create(TRIM(fileout)//'_wind10m.nc',nf90_clobber,ncidowind)
...
if(statuso /= nf90_NoErr) call handle_err(statuso)
WRITE(*,*) 'Output file ', TRIM(fileout)//'_wind10m.nc', ' is created.'

!T2M file
statuso = nf90_create(TRIM(fileout)//'_t2m.nc',nf90_clobber,ncidot2m)
if(statuso /= nf90_NoErr) call handle_err(statuso)
if(yes_no.eq.1)then
...
if(statuso /= nf90_NoErr) call handle_err(statuso)
WRITE(*,*) 'Output file ', TRIM(fileout)//'_t2m.nc', ' is created.'

!Q2M file
statuso = nf90_create(TRIM(fileout)//'_q2m.nc',nf90_clobber,ncidoq2m)
if(statuso /= nf90_NoErr) call handle_err(statuso)
if(yes_no.eq.1)then
...
if(statuso /= nf90_NoErr) call handle_err(statuso)
WRITE(*,*) 'Output file ', TRIM(fileout)//'_q2m.nc', ' is created.'

!Cloud file
statuso = nf90_create(TRIM(fileout)//'_cloud.nc',nf90_clobber,ncidocloud)
if(statuso /= nf90_NoErr) call handle_err(statuso)
if(yes_no.eq.1)then
...
statuso = nf90_put_att(ncidocloud, cloudairVarId,"units", &
                 " ")
if(statuso /= nf90_NoErr) call handle_err(statuso)
statuso = nf90_put_att(ncidocloud, cloudairVarId,"time", &
                 "time")
if(statuso /= nf90_NoErr) call handle_err(statuso)
if(yes_no.eq.0)then
...
WRITE(*,*) 'Output file ', TRIM(fileout)//'_cloud.nc', ' is created.'

!Rain file
statuso = nf90_create(TRIM(fileout)//'_rain.nc',nf90_clobber,ncidorain)
if(statuso /= nf90_NoErr) call handle_err(statuso)
if(yes_no.eq.1)then
...
WRITE(*,*) 'Output file ', TRIM(fileout)//'_rain.nc', ' is created.'

!Sward file
statuso = nf90_create(TRIM(fileout)//'_dsw.nc',nf90_clobber,ncidodsw)
if(statuso /= nf90_NoErr) call handle_err(statuso)
if(yes_no.eq.1)then
...
statuso = nf90_def_var(ncidodsw,"swrad",nf90_float, &
                 (/ XiRhoDimIDdsw, EtaRhoDimIDdsw, TimeOutDimIDdsw /), swradVarId)
if(statuso /= nf90_NoErr) call handle_err(statuso)
statuso = nf90_put_att(ncidodsw, swradVarId,"long_Name", &
...
WRITE(*,*) 'Output file ', TRIM(fileout)//'_dsw.nc', ' is created.'

!Sward file
statuso = nf90_create(TRIM(fileout)//'_dlw.nc',nf90_clobber,ncidodlw)
if(statuso /= nf90_NoErr) call handle_err(statuso)
if(yes_no.eq.1)then
...
statuso = nf90_def_var(ncidodlw,"lwrad_down",nf90_float, &
                 (/ XiRhoDimIDdlw, EtaRhoDimIDdlw, TimeOutDimIDdlw /), lwradVarId)
if(statuso /= nf90_NoErr) call handle_err(statuso)
...
if(statuso /= nf90_NoErr) call handle_err(statuso)
WRITE(*,*) 'Output file ', TRIM(fileout)//'_dlw.nc', ' is created.'


!MSL file
!---- Has different lons and lats dimensions.
statuso = nf90_create(TRIM(fileout)//'_msl.nc',nf90_clobber,ncidomsl)
if(statuso /= nf90_NoErr) call handle_err(statuso)
if(yes_no.eq.1)then
...
if(statuso /= nf90_NoErr) call handle_err(statuso)
WRITE(*,*) 'Output file ', TRIM(fileout)//'_msl.nc', ' is created.'
...
ALLOCATE(Pair(Lp_dim_msl,Mp_dim_msl)) 
...
!Open output file
statuso = nf90_open(trim(fileout)//'_wind10m.nc',nf90_write,ncidowind)
if(statuso /= nf90_NoErr) call handle_err(statuso)
statuso = nf90_open(trim(fileout)//'_msl.nc',nf90_write,ncidomsl)
if(statuso /= nf90_NoErr) call handle_err(statuso)
statuso = nf90_open(trim(fileout)//'_q2m.nc',nf90_write,ncidoq2m)
if(statuso /= nf90_NoErr) call handle_err(statuso)
statuso = nf90_open(trim(fileout)//'_t2m.nc',nf90_write,ncidot2m)
if(statuso /= nf90_NoErr) call handle_err(statuso)
statuso = nf90_open(trim(fileout)//'_cloud.nc',nf90_write,ncidocloud)
if(statuso /= nf90_NoErr) call handle_err(statuso)
statuso = nf90_open(trim(fileout)//'_rain.nc',nf90_write,ncidorain)
if(statuso /= nf90_NoErr) call handle_err(statuso)
statuso = nf90_open(trim(fileout)//'_dsw.nc',nf90_write,ncidodsw)
if(statuso /= nf90_NoErr) call handle_err(statuso)
statuso = nf90_open(trim(fileout)//'_dlw.nc',nf90_write,ncidodlw)
if(statuso /= nf90_NoErr) call handle_err(statuso)
...
if(status_wind /= nf90_NoErr) call handle_err(status_wind)
! Get wind forecast hour id
status_wind = nf90_inq_varid(ncid_wind, "forecast_hour",FCVarId_wind)
if(status_wind /= nf90_NoErr) call handle_err(status_wind)
! Get U10 var id
...
if(status_msl /= nf90_NoErr) call handle_err(status_msl)
! Get MSL forecast hour id
status_msl = nf90_inq_varid(ncid_msl, "forecast_hour",FCVarId_msl)
if(status_msl /= nf90_NoErr) call handle_err(status_msl)
! Get MSL var id
...
if(status_t2m /= nf90_NoErr) call handle_err(status_t2m)
! Get T2M forecast hour id
status_t2m = nf90_inq_varid(ncid_t2m, "forecast_hour",FCVarId_t2m)
if(status_t2m /= nf90_NoErr) call handle_err(status_t2m)
! Get T2M var id
...
if(status_q2m /= nf90_NoErr) call handle_err(status_q2m)
! Get Q2M forecast hour id
status_q2m = nf90_inq_varid(ncid_q2m, "forecast_hour",FCVarId_q2m)
if(status_q2m /= nf90_NoErr) call handle_err(status_q2m)
! Get Q2M var id
...
if(status_rain /= nf90_NoErr) call handle_err(status_rain)
! Get RAIN forecast hour id
status_rain = nf90_inq_varid(ncid_rain, "forecast_hour",FCVarId_rain)
if(status_rain /= nf90_NoErr) call handle_err(status_rain)
! Get RAIN var id
...
if(status_dlw /= nf90_NoErr) call handle_err(status_dlw)
! Get DLW forecast hour id
status_dlw = nf90_inq_varid(ncid_dlw, "forecast_hour",FCVarId_dlw)
if(status_dlw /= nf90_NoErr) call handle_err(status_dlw)
! Get DLW var id
...
if(status_dsw /= nf90_NoErr) call handle_err(status_dsw)
! Get DSW forecast hour id
status_dsw = nf90_inq_varid(ncid_dsw, "forecast_hour",FCVarId_dsw)
if(status_dsw /= nf90_NoErr) call handle_err(status_dsw)
! Get DSW var id
...
if(status_cloud /= nf90_NoErr) call handle_err(status_cloud)
! Get DSW forecast hour id
status_cloud = nf90_inq_varid(ncid_cloud, "forecast_hour",FCVarId_cloud)
if(status_cloud /= nf90_NoErr) call handle_err(status_cloud)
! Get CLOUD var id
...
status_wind = nf90_get_var(ncid_wind,FCVarId_wind,FC_wind,start = (/itime/))
if(status_wind /= nf90_NoErr) call handle_err(status_wind)
! Read in U10
...
!Read in Forecast time
status_msl = nf90_get_var(ncid_msl,FCVarId_msl,FC_msl,start = (/itime/))
if(status_msl /= nf90_NoErr) call handle_err(status_msl)
! Read in MSL values
...
!Read in Forecast time
status_t2m = nf90_get_var(ncid_t2m,FCVarId_t2m,FC_t2m,start = (/itime/))
if(status_t2m /= nf90_NoErr) call handle_err(status_t2m)
! Read in T2M values
...
!Read in Forecast time
status_q2m = nf90_get_var(ncid_q2m,FCVarId_q2m,FC_q2m,start = (/itime/))
if(status_q2m /= nf90_NoErr) call handle_err(status_q2m)
! Read in Q2M values
...
!Read in Forecast time
status_cloud = nf90_get_var(ncid_cloud,FCVarId_cloud,FC_cloud,start = (/itime/))
if(status_cloud /= nf90_NoErr) call handle_err(status_cloud)
!!!!!!!---------------------------------------------------------
! Adding or subtracting to make this the centre of the average
!!!!!!!---------------------------------------------------------
if (FC_cloud .eq. 6) then
   tday_cloud = tday_cloud - (3*3600)
else
   call write_err("cloud forecast time not 6")
end if
...
!Read in Forecast time
status_rain = nf90_get_var(ncid_rain,FCVarId_rain,FC_rain,start = (/itime/))
if(status_rain /= nf90_NoErr) call handle_err(status_rain)
!!!!!!!---------------------------------------------------------
! Adding or subtracting to make this the centre of the average
!!!!!!!---------------------------------------------------------
if (FC_rain .eq. 6) then
   tday_rain = tday_rain - (3*3600)
else
   call write_err("rain forecast time not 6")
end if
! Read in RAIN values
...
!Read in Forecast time
status_dsw = nf90_get_var(ncid_dsw,FCVarId_dsw,FC_dsw,start = (/itime/))
if(status_dsw /= nf90_NoErr) call handle_err(status_dsw)
!!!!!!!---------------------------------------------------------
! Adding or subtracting to make this the centre of the average
!!!!!!!---------------------------------------------------------
if (FC_dsw .eq. 6) then
   tday_dsw = tday_dsw - (3*3600)
else
   call write_err("dsw forecast time not 6")
end if
! Read in DSW values
...
!Read in Forecast time
status_dlw = nf90_get_var(ncid_dlw,FCVarId_dlw,FC_dlw,start = (/itime/))
if(status_dlw /= nf90_NoErr) call handle_err(status_dlw)
!!!!!!!---------------------------------------------------------
! Adding or subtracting to make this the centre of the average
!!!!!!!---------------------------------------------------------
if (FC_dlw .eq. 6) then
   tday_dlw = tday_dlw - (3*3600)
else
   call write_err("dlw forecast time not 6")
end if
! Read in DLW values
...
statuso = nf90_put_var(ncidowind,TimeOutVarIdwind,tday_wind,start=(/ irec /))
if(statuso /= nf90_NoErr) call handle_err(statuso)
statuso = nf90_put_var(ncidoq2m,TimeOutVarIdq2m,tday_q2m,start=(/ irec /))
if(statuso /= nf90_NoErr) call handle_err(statuso)
statuso = nf90_put_var(ncidot2m,TimeOutVarIdt2m,tday_t2m,start=(/ irec /))
if(statuso /= nf90_NoErr) call handle_err(statuso)
statuso = nf90_put_var(ncidocloud,TimeOutVarIdcloud,tday_cloud,start=(/ irec /))
if(statuso /= nf90_NoErr) call handle_err(statuso)
statuso = nf90_put_var(ncidorain,TimeOutVarIdrain,tday_rain,start=(/ irec /))
if(statuso /= nf90_NoErr) call handle_err(statuso)
statuso = nf90_put_var(ncidodsw,TimeOutVarIddsw,tday_dsw,start=(/ irec /))
if(statuso /= nf90_NoErr) call handle_err(statuso)
statuso = nf90_put_var(ncidodlw,TimeOutVarIddlw,tday_dlw,start=(/ irec /))
if(statuso /= nf90_NoErr) call handle_err(statuso)
statuso = nf90_put_var(ncidomsl,TimeOutVarIdmsl,tday_msl,start=(/ irec /))
if(statuso /= nf90_NoErr) call handle_err(statuso)

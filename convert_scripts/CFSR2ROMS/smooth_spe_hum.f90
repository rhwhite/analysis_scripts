...
INTEGER LonDimID, LatDimID, TimeDimId,TimeVarId, Q2MVarId
...
OPEN(unit=10,file='smooth_spe_hum.in')
READ(10,'(a)') pathin
READ(10,'(a)') pathout
...
DO imonth= month_st,12
   ...
   filein=TRIM(pathin)//'q2m.gdas.'//TRIM(ryear)//TRIM(rmonth)//'.grb2.nc'
   fileout=TRIM(pathout)//'q2m_'//TRIM(ryear)//TRIM(rmonth)//'.nc'
   ...
   DO itime= 1,ntime
      ...
      ! Read in Q2M value
      ! Get Q2M var id
      status_cfsr = nf90_inq_varid(ncid_cfsr, "SPF_H_L103",Q2MVarId)
      if(status_cfsr /= nf90_NoErr) call handle_err(status_cfsr)
      status_cfsr = nf90_get_var(ncid_cfsr,Q2MVarId,scr,start=(/1, 1,itime /))
      if(status_cfsr /= nf90_NoErr) call handle_err(status_cfsr)
      ...
      status_cfsr = nf90_put_var(ncid_cfsr,Q2MVarId,scr,(/1,1,itime/))
      if(status_cfsr /= nf90_NoErr) call handle_err(status_cfsr)   
   ENDDO
   ...
   if(status_cfsr /= nf90_NoErr) call handle_err(status_cfsr)
   write(*,*) 'Laplacian interpolation and Polar smoothing done for file: ','q2m_'//TRIM(ryear)//TRIM(rmonth)//'.nc'
ENDDO

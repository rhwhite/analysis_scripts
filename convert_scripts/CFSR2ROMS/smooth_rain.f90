...
INTEGER LonDimID, LatDimID, TimeDimId,TimeVarId, RAINVarId
...
OPEN(unit=10,file='smooth_rain.in')
READ(10,'(a)') pathin
READ(10,'(a)') pathout
...
DO imonth= month_st,12
   ...
   filein=TRIM(pathin)//'prate.gdas.'//TRIM(ryear)//TRIM(rmonth)//'.grb2.nc'
   ...
   DO itime= 1,ntime
      ...
      ! Read in RAIN value
      ! Get RAIN var id
      status_cfsr = nf90_inq_varid(ncid_cfsr, "PRATE_L1",RAINVarId)
      if(status_cfsr /= nf90_NoErr) call handle_err(status_cfsr)
      status_cfsr = nf90_get_var(ncid_cfsr,RAINVarId,scr,start=(/1, 1,itime /))
      if(status_cfsr /= nf90_NoErr) call handle_err(status_cfsr)
      ...
      status_cfsr = nf90_put_var(ncid_cfsr,RAINVarId,scr,(/1,1,itime/))
      if(status_cfsr /= nf90_NoErr) call handle_err(status_cfsr)   
   ENDDO
   ...
   if(status_cfsr /= nf90_NoErr) call handle_err(status_cfsr)
   write(*,*) 'Laplacian interpolation and Polar smoothing done for file: ','rain_'//TRIM(ryear)//TRIM(rmonth)//'.nc'
ENDDO

...
INTEGER LonDimID, LatDimID, TimeDimId,TimeVarId, T2MVarId
...
OPEN(unit=10,file='smooth_tair.in')
READ(10,'(a)') pathin
READ(10,'(a)') pathout
...
DO imonth= month_st,12
   ...
   filein=TRIM(pathin)//'tmp2m.gdas.'//TRIM(ryear)//TRIM(rmonth)//'.grb2.nc'
   fileout=TRIM(pathout)//'t2m_'//TRIM(ryear)//TRIM(rmonth)//'.nc'
   ...
   DO itime= 1,ntime
      ...
      ! Read in T2M value
      ! Get T2M var id
      status_cfsr = nf90_inq_varid(ncid_cfsr, "TMP_L103",T2MVarId)
      if(status_cfsr /= nf90_NoErr) call handle_err(status_cfsr)
      status_cfsr = nf90_get_var(ncid_cfsr,T2MVarId,scr,start=(/1, 1,itime /))
      if(status_cfsr /= nf90_NoErr) call handle_err(status_cfsr)
      ...
      status_cfsr = nf90_put_var(ncid_cfsr,T2MVarId,scr,(/1,1,itime/))
      if(status_cfsr /= nf90_NoErr) call handle_err(status_cfsr)   
   ENDDO
   ...
   if(status_cfsr /= nf90_NoErr) call handle_err(status_cfsr)
   write(*,*) 'Laplacian interpolation and Polar smoothing done for file: ','t2m_'//TRIM(ryear)//TRIM(rmonth)//'.nc'
ENDDO

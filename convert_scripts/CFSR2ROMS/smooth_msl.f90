...
INTEGER LonDimID, LatDimID, TimeDimId,TimeVarId, MSLVarId
...
OPEN(unit=10,file='smooth_msl.in')
READ(10,'(a)') pathin
READ(10,'(a)') pathout
...
DO imonth= month_st,12
   ...
   filein=TRIM(pathin)//'prmsl.gdas.'//TRIM(ryear)//TRIM(rmonth)//'.grb2.nc'
   fileout=TRIM(pathout)//'msl_'//TRIM(ryear)//TRIM(rmonth)//'.nc'
   ...
   DO itime= 1,ntime
      ...
      ! Read in MSL value
      ! Get MSL var id
      status_cfsr = nf90_inq_varid(ncid_cfsr, "PRMSL_L101",MSLVarId)
      if(status_cfsr /= nf90_NoErr) call handle_err(status_cfsr)
      status_cfsr = nf90_get_var(ncid_cfsr,MSLVarId,scr,start=(/1, 1,itime /))
      if(status_cfsr /= nf90_NoErr) call handle_err(status_cfsr)
      ...
      status_cfsr = nf90_put_var(ncid_cfsr,MSLVarId,scr,(/1,1,itime/))
      if(status_cfsr /= nf90_NoErr) call handle_err(status_cfsr)   
   ENDDO
   ...
   if(status_cfsr /= nf90_NoErr) call handle_err(status_cfsr)
   write(*,*) 'Laplacian interpolation and Polar smoothing done for file: ','msl_'//TRIM(ryear)//TRIM(rmonth)//'.nc'
ENDDO

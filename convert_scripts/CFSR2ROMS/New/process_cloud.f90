...
DO imonth= month_st,12
   ...
   filein=TRIM(pathin)//'cldcovtot.gdas.'//TRIM(ryear)//TRIM(rmonth)//'.grb2.nc'
   ...
   DO itime= 1,ntime
      ...
      ! Read in value
      ! Get var id
      status_cfsr = nf90_inq_varid(ncid_cfsr, "T_CDC_L200",VarId)
      if(status_cfsr /= nf90_NoErr) call handle_err(status_cfsr)
   ...
   if(status_cfsr /= nf90_NoErr) call handle_err(status_cfsr)
   write(*,*) 'Check for missing values and time in hours added for: ','cloud_'//TRIM(ryear)//TRIM(rmonth)//'.nc'
ENDDO

...
DO imonth= month_st,12
   ...
   DO itime= 1,ntime
      ...
      ! Read in value
      ! Get var id
      status_cfsr = nf90_inq_varid(ncid_cfsr, "DLWRF_L1",VarId)
      if(status_cfsr /= nf90_NoErr) call handle_err(status_cfsr)
   ...
   if(status_cfsr /= nf90_NoErr) call handle_err(status_cfsr)
   write(*,*) 'Check for missing values and time in hours added for: ','dlw_'//TRIM(ryear)//TRIM(rmonth)//'.nc'
ENDDO
